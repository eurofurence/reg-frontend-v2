...
const apiFetch = async (path: string, init?: RequestInit) => {
  ...
  if (!res.ok) {
    ...
    throw new AttSrvAppError(message || "API Error", {
...
export class AttSrvAppError extends Error {
  ...
  constructor(
    message: string,
    opts?: { code?: string | null; status?: number; body?: any }
  ) {
    super(message);
...
const attendeeDtoFromRegistrationInfo = (registrationInfo: any) => {
  ...
  packagesMap.set(
    "attendance",
    registrationInfo.ticketType?.type === "full" ? 1 : 0
  );

  // day tickets mapping using interval
  if (
    registrationInfo.ticketType?.type === "day" &&
    registrationInfo.ticketType.day
  ) {
  ...
  packagesMap.set(
    "sponsor2",
    registrationInfo.ticketLevel?.level === "super-sponsor" ? 1 : 0
  );
  ...
  packagesMap.set(
    ...
    level &&
      !(
        (config.ticketLevels as any)[level]?.includes?.includes("stage-pass") ??
        false
      ) &&
  ...
  packagesMap.set(
    ...
    level &&
      !(
        (config.ticketLevels as any)[level]?.includes?.includes("tshirt") ??
        false
      ) &&
  ...
  const hiddenAddons = Object.fromEntries(
    Object.entries(config.addons)
      ...
      .map(([id, _]: [string, any]) => [
        id,
        { selected: packagesMap.has(id), options: {} },
      ])
  );
  ...
  const addons: any = {
    ...
    "stage-pass": {
      selected:
        ((config.ticketLevels as any)[level]?.includes?.includes(
          "stage-pass"
        ) ??
          false) ||
    ...
    tshirt: {
      ...
      options: {
        size: tshirtFromApi(
          registrationInfo.ticketLevel?.addons?.tshirt?.options?.size ?? null
        ),
      },
    },
  ...
  return {
    ...
    spoken_languages: (
      registrationInfo.personalInfo.spokenLanguages ?? []
    ).join(","),
...
export const registrationCountdownCheck = async () =>
  apiFetch("/countdown", { method: "GET" });

export const submitRegistration = async (registration: Registration) =>
  apiFetch("/attendees", {
    method: "POST",
    body: JSON.stringify(
      attendeeDtoFromRegistrationInfo(normalizeRegistration(registration))
    ),
  });

export const findMyRegistrations = async () =>
  apiFetch("/attendees", { method: "GET" });
...
const normalizeRegistration = (reg: Registration) => {
...
export const findExistingRegistration = async () => {
  ...
  try {
    ...
    if (
      ...
      message.includes("attendee.owned.notfound")
    ) {
      return undefined;
    }
    throw err;
...
const registrationInfoFromAttendeeDto = (attendeeDto: any) => {
  const packagesMap = new Map(
    (attendeeDto.packages_list || []).map((e: any) => [e.name, e.count])
  );
  const flags = new Set((attendeeDto.flags || "").split(",").filter(nonEmpty));
  const options = new Set(
    (attendeeDto.options || "").split(",").filter(nonEmpty)
  );
  ...
  return {
    ...
    personalInfo: {
      ...
      dateOfBirth: attendeeDto.birthday
        ? DateTime.fromISO(attendeeDto.birthday)
        : undefined,
      spokenLanguages: (attendeeDto.spoken_languages || "")
        .split(",")
        .filter(nonEmpty),
